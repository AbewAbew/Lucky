...
# Kept in sync by hand with sql/schema.sql, which is the copy-paste version
# for the Neon SQL editor. This copy is what the app runs on startup
# (idempotent — CREATE TABLE/INDEX IF NOT EXISTS), so a fresh environment
# works even if nobody ran the SQL file first.
SCHEMA = """
...
def _get_pool() -> ConnectionPool:
    """Lazily build (and rebuild, if the target changes) a pooled connection.

    Tests replace `db.settings` wholesale with an object pointing at the same
    Neon database but a different `db_schema` (an isolated `pytest` schema,
    never the app's real `public` schema), so the pool is keyed on both the
    URL and the schema and rebuilt whenever either changes.
    """
    ...
    if _pool is None or _pool_key != key:
        ...
        if schema != "public":
            # Created once, up front, on a single throwaway connection — doing
            # this inside the pool's `configure` callback instead would let
            # several warmup connections race CREATE SCHEMA IF NOT EXISTS
            # against each other and intermittently fail.
            with psycopg.connect(settings.database_url) as setup_connection:
                setup_connection.execute(f"CREATE SCHEMA IF NOT EXISTS {schema}")
...
@contextmanager
def transaction(*, immediate: bool = False) -> Iterator[psycopg.Connection]:
    """`immediate` is accepted only for call-site compatibility with the old
    SQLite `BEGIN IMMEDIATE` semantics and has no effect here: Postgres's
    MVCC allows concurrent writers by default (unlike SQLite's single-writer
    model), so call sites that need protection from a real race rely on this
    table's own UNIQUE constraints instead (see join_cards in repository.py).
    """
    with _get_pool().connection() as connection:
        yield connection
